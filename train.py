...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, notest, nosave, workers, fold, weights= \
        opt.save_dir, opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.notest, opt.nosave, opt.workers, opt.fold, opt.weights


    imsize = opt.img_size
    ROOT_PATH = "gdrive/My Drive/Colab Notebooks/covid19-yolo-save"
    # Directories
    
    save_dir = Path(ROOT_PATH)
    last = save_dir / f'{weights.split(".")[0]}_last_fold{str(fold)}_imsize{str(imsize)}.pt'
    best = save_dir / f'{weights.split(".")[0]}_best_fold{str(fold)}_imsize{str(imsize)}.pt'
    results_file = save_dir / f'{weights.split(".")[0]}_results_fold{str(fold)}_imsize{str(imsize)}.txt'
...
def parse_opt(known=False):
    ...
    parser.add_argument('--fold', type=int, default=0, help='train_fold_number')
